...
def incomeStatement(request, id):
    ...
    if request.method == 'POST':
        ...
        if monthly_date and monthly_month_name:
            ...
            month_or_quarter_or_year_name = f"{monthly_month_name} {datetime.strptime(monthly_date, '%Y-%m-%d').year}"
            ...
            month_or_quarter_or_year_name = f"{quarterly_quarter_value} {datetime.strptime(quarterly_date, '%Y-%m-%d').year}"
...
def balanceSheet(request, id):
    ...
    if request.method == 'POST':
        ...
        if balance_sheet_monthly_month_name and balance_sheet_monthly_month_name:
            date = balance_sheet_monthly_date
            monthly_or_quarterly_or_yearly = 'monthly'
            month_or_quarter_or_year_name = f"{balance_sheet_monthly_month_name} {datetime.strptime(balance_sheet_monthly_date, '%Y-%m-%d').year}"
            end_date = monthly_end_date

        elif balance_sheet_quarterly_date and balance_sheet_quarterly_quarter_value:
            date = balance_sheet_quarterly_date
            end_date = quarterly_end_date
            monthly_or_quarterly_or_yearly = 'quarterly'
            month_or_quarter_or_year_name = f"{balance_sheet_quarterly_quarter_value} {datetime.strptime(balance_sheet_quarterly_date, '%Y-%m-%d').year}"

        elif balance_sheet_yearly_date and balance_sheet_yearly_year:
...
def get_next_period_headers(start_period, period_type):
    ...
    financial_year_months = ["April", "May", "June", "July", "August", "September", "October", "November", "December", "January", "February", "March"]
    
    if period_type == 'monthly':
        # Parse and clean the start_period to handle common input issues
        start_period = start_period.strip().title()  # Remove extra spaces and ensure title case

        # Split to extract month and year, defaulting to the current year if year is missing
        parts = start_period.split()
        if len(parts) == 1:
            start_month = parts[0]
            year = datetime.now().year  # Default to current year if not provided
        elif len(parts) == 2:
            start_month, year = parts
            year = int(year)  # Convert year to integer for calculation
        else:
            raise ValueError(f"Invalid start_period format: {start_period}. Expected 'Month Year'.")

        # Validate the month
        try:
            start_index = financial_year_months.index(start_month)
        except ValueError:
            raise ValueError(f"Invalid start_period: {start_period}. Ensure the month name is valid and correctly spelled (e.g., 'January').")

        # Initialize headers with the selected period as the first item
        headers = [f"{financial_year_months[start_index]} {year}"]

        # Generate the next 7 periods after the selected one in 'Month Year' format
        for i in range(1, 8):  # Start from the next month for growth rates
            index = (start_index + i) % 12
            adjusted_year = year + ((start_index + i) // 12)  # Adjust year dynamically
            headers.append(f"{financial_year_months[index]} {adjusted_year}")
        return headers

    elif period_type == 'quarterly':
        # Define quarters aligned with the financial year
        financial_year_quarters = ["Q1 Apr,May,Jun", "Q2 Jul,Aug,Sep", "Q3 Oct,Nov,Dec", "Q4 Jan,Feb,Mar"]
        
        start_period = start_period.strip().title()  # Remove extra spaces and ensure title case
        parts = start_period.split()
        if len(parts) == 1:
            start_quarter = parts[0]
            year = datetime.now().year  # Default to current year if not provided
        elif len(parts) == 2:
            start_quarter, year = parts
            year = int(year)  # Convert year to integer for calculation
        else:
            raise ValueError(f"Invalid start_period format: {start_period}. Expected 'QX Year'.")

        # Validate the quarter
        try:
            start_index = financial_year_quarters.index(start_quarter)
        except ValueError:
            raise ValueError(f"Invalid start_period: {start_period}. Ensure the quarter is in financial year format (e.g., 'Q1').")

        # Initialize headers with the selected period as the first item
        headers = [f"{financial_year_quarters[start_index]} {year}"]

        # Generate headers for 7 periods after the selected one in 'Quarter Year' format
        for i in range(1, 8):  # Start from the next quarter for growth rates
            index = (start_index + i) % 4
            adjusted_year = year + ((start_index + i) // 4)
            headers.append(f"{financial_year_quarters[index]} {adjusted_year}")
        return headers

    elif period_type == 'yearly':
        start_period = start_period.strip()
        try:
            start_year = int(start_period)  # Convert to integer directly
        except ValueError:
            raise ValueError(f"Invalid start_period: {start_period} is not a valid year.")

        # Initialize headers with the selected year as the first item
        headers = [str(start_year)]

        # Generate headers for the next 7 years after the selected one
        headers += [str(start_year + i + 1) for i in range(7)]
        return headers
